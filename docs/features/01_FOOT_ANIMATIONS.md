...
- **Target Behavior (Athletic Compression & Lateral Pre-Wind):**
  ...
  - **Directional Lateral Pop Synergy:** When loading pop with horizontal stick deflection ($|\text{stick}.x| > 0.15$ for directional jumping), visually shift the popping foot across the tail width ($\pm 0.03\text{m}$ along local $X$). This visually telegraphs lateral jumping intention before takeoff!
...
2. **Presentation Sovereignty (Rule #6):** As documented in `FootRig.gd` header notes, this node is strictly for presentation. No physical collision check, ground probe, or velocity calculation may ever read a foot animation position to determine gameplay success or failure.
...
5. **Stance Integrity & `update_stance_facts()` Guarantee:** Even during extreme diagonal flick extensions or lateral tail scoops, the leading foot must never cross behind `BoardPivot` center ($Z=0$), and the trailing foot must never advance ahead of center. This guarantees `FootInputState.update_stance_facts()` evaluates identical leading/trailing stance classification without oscillation or switch confusion.
6. **Complete Rest Transform Tracking:** When entering `_ready()`, `FootRig` must capture complete rest transforms (`position` AND `rotation`) so `settle()`, `hover()`, and `lower()` smoothly lerp shoe box orientations back to horizontal zero upon touchdown or recovery.
