...
- **`res://scripts/player/FootRig.gd`** (under `BoardPivot`): shoe boxes, ankle pegs, and every animation that moves them. **Presentation only, and now structurally so** — nothing outside the file reads a live foot position, so an animation may put a shoe anywhere at all.
  ...
  - **Rule — the airborne tuck is a PARABOLA, parameterised on flip progress.** The rider has jumped; once airborne nothing acts on them but gravity, so the feet travel under CONSTANT acceleration relative to the deck - `4p(1-p)`, leaving with upward speed, peaking halfway, and arriving at the speed they left with. This replaced `flip_hover_height`, a fixed offset the shoe sprang to and then sat at motionless for ~10 frames before dropping: free fall does not do that, and it read exactly as it was, the feet levitating and then stomping. A sine hump is the tempting near-miss and is also wrong - it puts its greatest acceleration at the apex. **Consequences:** the foot arrives with real downward speed, so the plant is in the arc and needs no separate stomp event; and there is nothing to stagger between the two feet, which are on one body and leave and return together (a `stomp_stagger` timer that froze one foot mid-air while the other dropped read as robotic, correctly). Parameterised on `_flip_progress()` rather than wall-clock, so the feet are back on the deck exactly as the griptape comes round for any trick duration.
